...
The BONMI_package.R script is designed for efficient matrix transformation and dimensionality reduction, particularly tailored for analyzing Positive Pointwise Mutual Information (PPMI) matrices. It features the [Procrustes](procrustes.qmd) function, which aligns two matrices by computing an orthogonal transformation matrix that minimizes their Frobenius norm difference. The [embedding](embedding.qmd) function extracts a low-dimensional representation from singular value decomposition (SVD) results to simplify complex data. The main function, [BONMI](bonmi.qmd), processes a list of PPMI matrices, optionally applies weights, and leverages standard or randomized SVD to create reduced-rank embeddings. It aligns these embeddings with the Procrustes function, ensuring consistent representation across datasets.
